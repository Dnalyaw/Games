...
def main():
    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
        if s.head.pos[0] > snack.pos[0] and not s.right:
        ...
        elif s.head.pos[0] < snack.pos[0] and not s.left:
            s.dirnx = 1  # x goes down by one when going left
            s.dirny = 0
            s.right = True
            s.left = False
        ...
        elif s.head.pos[1] > snack.pos[1] and not s.down:
        ...
        elif s.head.pos[1] < snack.pos[1] and not s.up:
            s.dirnx = 0  # x goes down by one when going left
            s.dirny = 1
            s.right = False
            s.left = False
